...
## Functional Specification v0.1 — Phase 16 提案（草稿）

> 日期：2026-06-11 ｜ 前置文件：`ROUTING_PIPELINE_DIAGNOSIS.md`（附錄 A）、`FUNCTIONAL_SPEC_v1.2.md`
> 狀態：規劃中，待審閱
...
`ROUTING_PIPELINE_DIAGNOSIS.md` 確認自製 GTFS graph router 的問題屬於**演算法類別性缺陷**：
...
- G4：診斷報告的三類 bug 在 OTP 路徑上**結構性消失**（驗收條件 §11）。
...
新檔 `src/service/otp-routing.service.ts`，介面對齊現有 planner：
...
### 8.1 a11y enrichment 上移（連動診斷報告問題四）
